-- Active: 1732688631660@@127.0.0.1@3306@sesac
show databases;
...
INSERT INTO user(name, age, address) VALUES ('한소희', 25, '서울특별시 관악구');
INSERT INTO user(name, age, address) VALUES ('정세희', 19, '부산광역시 해운대구');
INSERT INTO user(name, age, address) VALUES ('이형석', 23, '서울특별시 노원구');
INSERT INTO user(name, age, address) VALUES ('김성민', 32, '강웓도 강릉시');
INSERT INTO user(name, age, address) VALUES ('박수진', 37, '충청남도 공주시');
INSERT INTO user(name, age, address) VALUES ('권순모', 26, '서울특별시 영등포구');
INSERT INTO user(name, age, address) VALUES ('진현정', 40, '강원도 속초시');
INSERT INTO user(name, age, address) VALUES ('권희수', 36, '서울특별시 강서구');

-- Read >> SELECT [칼럼 이름] FROM [WHERE]
SELECT * FROM user; -- 전체 조회

SELECT name FROM user; -- 특정 칼럼 전체 조회
...
-- 서울시에 사는 사람의 이름 2개만 조회
SELECT name FROM user WHERE address LIKE '%서울%' ORDER BY name DESC LIMIT 2;
...
UPDATE user SET address="서울 특별시 도봉구" WHERE id=1;

UPDATE user SET address="제주특별자치도 제주시", name="이지현" WHERE id=3;
